# DARKVIEW BY ASTROMAN — ENGINEERING STANDARDS

The rules this repository is built to. Where a decision needed reasoning rather
than a rule, it is recorded in `docs/decisions/` and this file defers to it.
...
A material conflict between these documents is settled in a dated decision record,
never silently in the code. Where an earlier plan has been deliberately superseded,
there is a record in `docs/decisions/` saying so — this file does not outrank a
...
recorded in `docs/decisions/ADR-001-camera-control-path.md`. Reversing it is a
maintainer decision, not an implementation one: **SharpCap, OBS and every other
screen-capture path stay out of the product data flow.** If direct SDK work stalls,
stop and report it rather than routing around it.
...
The legal transitions between these states live in `apps/realtime/src/link/store.ts`,
read off the agent's own state machine, and both store implementations apply the same
table. A reported state that could not have followed the one the mission is in is
...
- No unattended or background process may command the real mount or camera.
...
- A **partner** observatory may operate unattended only while `APPROVED` under
  ...
  recorded horizon mask, a supervised first light, Park proven on that
  hardware and a fitted sky sensor. It runs unattended only in ADR-024's
  `UNATTENDED` posture, armed at the observatory and never by the cloud. It
  returns to refusing everything the moment any of those stops holding, and an
  operator can suspend it or revoke its token in one row.
- A **queued capture** on a first-party observatory may run unattended only
  ...
  and a fitted sky sensor; arming is a named operator act at the observatory.
  The agent enforces it locally, refuses the daylight override while
  unattended, and disarms itself on any hardware error or loss of sky data
...
- On heartbeat loss, device fault or operator abort: stop capture, halt unsafe
  motion, Park — in that order.
- `MAX_ALT_SAFE` is **measured** from the physical optical train. Never guess it,
  never let a default value ship.
- The Sun exclusion and the daylight lock have floors in code on both sides, not
  only bounds in the schema. A configured value cannot open them. See
  `docs/SAFETY.md`.
...
- Never backdate a commit, and never rewrite `main`. Real history only.
- Changes reach `main` through a reviewed pull request with green CI, squashed.
- Never deploy to production or run a production migration without the maintainer
  asking for it explicitly.
- Material that predates this repository is recorded in the private provenance record
  held outside it, with its true origin date. Pre-existing work is never presented as
  new development.
- Commits carry one author: the maintainer. No `Co-Authored-By:` trailers — the
  `commit-msg` hook in `.githooks/` enforces it, and a fresh clone enables it with
...
8. Work stays on its branch until reviewed.
