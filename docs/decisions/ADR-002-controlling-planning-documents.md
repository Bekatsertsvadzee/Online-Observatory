...
which contradict each other and the current architecture. `docs/ENGINEERING.md` requires that where
...
the **older** document, and `docs/ENGINEERING.md` is younger than every plan in the folder.

## Decision

### The order that governs

As stated in `docs/ENGINEERING.md`, and repeated here so it is discoverable from the decisions
folder:
...
| --- | --- |
| Ubuntu + **INDI** on the mini-PC | Windows + **ASCOM Alpaca** over HTTP on `127.0.0.1` (`docs/ENGINEERING.md`) |
...
- Any future supersession needs its own dated record here. `docs/ENGINEERING.md` must not silently
